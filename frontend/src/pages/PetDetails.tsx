...
import { ArrowLeft, Phone, Heart, Activity, Ruler, Clock, MapPin } from 'lucide-react'
...
const PetDetails: React.FC = () => {
  ...
  const selectedPetData = petsData?.find((pet) => pet.id === id)
  const selectedPetOrgData = orgsData?.orgsList?.find(
    (org) => org.id === selectedPetData?.org_id,
  )
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4">
              <div className="bg-white rounded-2xl p-6 border sticky top-24">
                ...
                <div className="mb-6">
                  <p className="font-medium text-lg">{selectedPetOrgData?.name}</p>
                  <div className="flex items-center gap-2 text-foreground/70">
                    <MapPin className="h-4 w-4" />
                    <span>
                      {selectedPetOrgData?.city && formatCityName(selectedPetOrgData?.city)}, {selectedPetOrgData?.state}
                    </span>
                  </div>
                </div>
